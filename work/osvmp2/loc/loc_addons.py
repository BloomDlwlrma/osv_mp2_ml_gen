...
def atomic_pops(self, mol, mo_coeff, method='meta_lowdin'):
    ...
    #self.t_chg += time_now() - t0
    #print_time(['POPS!!!', time_now() - t0])
    self.csc = csc
    #self.pop = proj
    return proj
...
def kernel(self, mo_coeff=None, pop_method=None, callback=None, verbose=3):
    ...
    #setattr(self, 't_chg', np.zeros(2))
    #self.t_chg = np.zeros(2)
    ...
    #cput01 = print_time(['initial guess of uo', time_elapsed(cput0)], log)
    ...
    cput1 = print_time(['initializing CIAH', time_elapsed(cput0)], log)
    ...
    for imacro in range(self.max_cycle):
        ...
        #cput1 = print_time(['cycle= %d'%(imacro+1), time_elapsed(cput1)], log)
...
def localization(mol, mo_coeff, local_type=1, pop_method='low_melow', grad_cal=True, 
                 use_sl=False, frozen=False, loc_fit=False, verbose=3, save_chk=True, 
                 log=None):
    def loc_mo(mo_coeff):
        if local_type == 1:
            ...
            uo = multi_dot([mo_coeff.T, s_mat, uc])
        return uo
    def get_uo():
        ...
        if (use_sl and ncore != 0) or (frozen):
            ...
            uo[ncore:, ncore:] = loc_mo(mo_coeff[:, ncore:])
            if (frozen == False):
                if ncore == 1:
                    uo[:ncore, :ncore] = 1
                else:
    ...
    if irank == 0:
        ...
        verbose = 0
    if grad_cal and loc_fit and frozen:
        use_sl = True
        frozen = False
    ncore = get_ncore(mol)
...
def moco_fit(mol, mo_coeff, lmo_close, nlmo_close, frozen=False):
    ...
    win_moco_fit, moco_fit = get_shared((nao, nocc))
...
def get_bfit_domain(mol, auxmol, aux_ratio, bfit_tol, use_group=False):
    nocc = mol.nelectron//2
    mo_list = range(nocc)
    atom_close = [[] for i in mo_list]
    atom_mos = [[] for atm in range(auxmol.natm)]
    aux_atm_offset = auxmol.offset_nr_by_atom()
    ...
    for atm in range(auxmol.natm):
        if (atm == 0) or (seg_mo == atom_mos[atm]):
            ...
            mos_diff = [i for i in atom_mos[atm] if i not in seg_mo]
            if ((len(mos_diff) < 10) and (abs(len(atom_mos[atm])-len(seg_mo)) < 10)):
                seg_atm.append(atm)
                seg_mo.extend(mos_diff)
                for i in atom_mos[atm]:
                    seg_act[i].append(atm)
            else:
...
def joint_fit_domains_by_aux(auxmol, mo_list, fit_close, joint_type='union'):
    #union or intersection fitting domains
    try:
        fit_joint = set(fit_close[mo_list[0]])
    except TypeError:
        print(fit_close[mo_list[0]])

    for j in mo_list[1:]:
        if joint_type == 'union':
            fit_joint = fit_joint.union(fit_close[j])
        else:
            fit_joint = fit_joint.intersection(fit_close[j])
    naux_joint = len(fit_joint)
    fit_joint = list2seg(fit_joint)
    return fit_joint, naux_joint
...
def create_mep(mol, outfile, dm, nx=40, ny=40, nz=40, resolution=None,
    ...
    print_time(["get cc", time_elapsed(t1)], log=log)
    ...
    win_vnuc, vnuc_node = get_shared(npoint, dtype='f8')
    ...
    win_vele, vele_node = get_shared(npoint, dtype='f8')
    nao = dm.shape[0]
    ...
    if irank == 0:
        ...
        print_time(["get mep", time_elapsed(t1)], log=log)

        t1 = time_now()
        # Write the potential
        cc.write(MEP, outfile, 'Molecular electrostatic potential in real space')
        print_time(["write mep", time_elapsed(t1)], log=log)
    else:
        MEP = None
    comm.Barrier()
...
def create_density(mol, outfile, dm, nx=40, ny=40, nz=40, resolution=None,
    ...
    #rho = np.empty(ngrids)
    win_rho, rho_node = get_shared(ngrids, dtype='f8')
    if job_slice is not None:
...
def analysis(mol, dm, meth='RHF', charge_method="meta_lowdin", save_data=None, 
    ...
    print_time(["pop analysis", time_elapsed(t1)], log=log)