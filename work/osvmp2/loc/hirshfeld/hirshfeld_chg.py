...
from osvmp2.osvutil import time_now, time_elapsed, print_time, get_slice, get_mem_spare, get_buff_len
from osvmp2.ga_addons import get_shared, free_win, Acc_and_get_GA, fence_and_free
...
def calc_rho(crds, mol, dm):
    ...
    #rho = np.empty(ncrds)
    win_rho, rho_node = get_shared(ncrds)
...
class Hirshfeld:
    ...
    def __init__(self, mol, dm, functional="b3lyp", get_cm5=True):
        ...
        #print_time([f"PART 1", time_elapsed(tt)])
        ...
        if irank_shm == 0:
            #print_time([f"PART 2", time_elapsed(t1)])
        ...
        #print_time([f"TOTAL rho time", time_elapsed(t1)])
        if irank_shm == 0:
            ...
            #print_time([f"TOTAL HIRSHFELD", time_elapsed(tt)])
