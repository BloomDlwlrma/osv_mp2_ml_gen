...
def derivative_R(self, T_matrix, T_bar, S_matrix_cp, F_matrix_cp):
    ...
    if twob_remote:
        ...
        self.t_dot = np.zeros(2)
        #self.lg_correct = True
        if (self.lg_correct):
            log = lib.logger.Logger(self.stdout, self.verbose)
            self.t_r_remote, self.t_r_close = 0, 0
            
            for ipair in self.pairlist:
                t0 = time_now()
                N = residue_eval(ipair, N)
                if (self.if_remote[ipair]):
                    self.t_r_remote += time_now() - t0
                else:
                    self.t_r_close += time_now() - t0
            
            time_list = [['dR/dQ for close pairs', self.t_r_close], ['dR/dQ for remote pairs', self.t_r_remote],
                        ['dot product', self.t_dot]]
            print_time(time_list, log)
        else:
            pairlist = self.pairlist_close
            if self.use_mbe:
                if (self.clus_type == 3) and (self.mbe_mode != 1):
                    pairlist = []
                    for ipair in self.pairlist:
                        if ipair//self.no != ipair%self.no:
                            pairlist.append(ipair)
    ...
    if self.use_mbe:
        if (self.clus_type == 3) or (self.clus_type == 21):
            ...
            #if (self.clus_type == 21): #or (self.mbe_mode != 1):
