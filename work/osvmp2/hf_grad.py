...
def grad_hcore_nuc(self, dm0, dme0, Pi=None, W=None, log=None):
    ...
    if self.mol_mm is not None:
        ...
        h_qmmm = None
        print_time(["QMMM grad", time_elapsed(t0)], log)
    comm.Barrier()
    if self.nnode > 1:
        Acc_and_get_GA(var=hcore)
    win_grad, grad_node = get_shared(len(self.atom_list)*3)
    comm_shm.Barrier()
    ...
    for win_i in [win_dm_tmp, win_dme_tmp, win_hcore, win_grad]:
        free_win(win_i)
    print_time(["core hamiltonian grad", time_elapsed(t1)], log)
    return grad
...
def kernel(self):
    ...
    get_ialp(self, self.with_df, 'hf', log)
