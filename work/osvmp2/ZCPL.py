...
from osvmp2 import hf_grad, CPOSV
...
def MO_basis_A(self, T_matrix):
    ...
    if (self.use_cposv):
        ...
        if self.lg_dr:
            ...
            self.lg_correct = False
        N = CPOSV.derivative_R(self, T_matrix, T_bar, S_matrix_cp, F_matrix_cp)
        self.lg_correct = True
        print_time(['derivative residue', time_elapsed(t0)], log)
    else:
        N = None
    t0 = time_now()
    ...
    def DM_to_AO(self, DMP2, T_bar):
        ...
        self.t_ij_close, self.t_ij_remote = np.zeros(2), np.zeros(2)
        self.t_ab_close, self.t_ab_remote = np.zeros(2), np.zeros(2)
    ...
    print_time(['Density matrix calculation done', time_elapsed(t0)], log)
    print_time(['OSV gradient in MO-basis finished', time_elapsed(tt)], log)
    self.t_g_mo = time_now() - t_g_mo
...
def AO_basis(self):
                            ...
                            for j in range(self.nocc_core, self.no):
                                if (i == j):
                                    ...
                                    if (abs(self.eo[j]-self.eo[i]) < 1.0e-7):
                                        ...
                                        self.DMP2[j, i] += theta[j, i]/(self.eo[i]-self.eo[j])
            if ztype == 1:
                if (self.use_cposv is not False):# and 1==0:
    ...
    def Z_vector_get_veff(dm=None, z_bej=None):
        ...
        def cal_jk(num, ialp_i, eri_tmp, jmat, kmat):
            ...
            self.t_j += time_now() - t0
            t0 = time_now()
            kmat += multi_dot([ialp_i.T, dm, eri_tmp]).T
            self.t_k += time_now() - t0
            return jmat, kmat
        t_k = time_now()
        win_jmat, j_mat = get_shared((self.nao, self.no), dtype='f8')
        win_kmat, k_mat = get_shared((self.nao, self.no), dtype='f8')
        split_z = False
        if dm is None:
            split_z = True
        if self.direct_int:
            ...
            def get_k_gammaq(k_mat=None):
                tt = time_now()
                ao_slice, shell_slice_rank = int_prescreen.get_slice_rank(mol, self.RHF.shell_slice, aslice=True)
                if split_z:
                    self.address_uip = []
                    ...
                    #shell_slice_rank = int_prescreen.get_slice_rank(mol, self.RHF.shell_slice, aslice=True)
            ...
            if split_z:
                ...
                self.t_ijpz += time_now() - t1
                #if self.loc_fit==False:
                #t1 = time_now()
                get_k_gammaq()
                #self.t_kga += time_now() - t1

                t1 = time_now()
                j_tmp, k_tmp = get_jk(j_mat, k_mat, gammaq_node)
                self.t_jk += time_now() - t1
                #j_tmp = get_j(j_mat)
            else:
                get_uip()
        ...
        if irank_shm != 0:
            ...
            if k_tmp is not None:
                Accumulate_GA_shm(win_jmat, k_mat, k_tmp)
    ...
    def Z_vector(feri):
        ...
        def krylov(aop, b, x0=None, tol=1e-6, max_cycle=30, x1=None, dot=ddot,
            ...
            comm_shm.Barrier()
            win_conv, conv = get_shared(1, dtype='i')
        ...
        def Z_vector_get_B():#            
            ...
            win_b0, B0 = get_shared((self.nv, self.no), dtype='f8')
            if irank_shm == 0:
        ...
        self.t_dm_send = np.zeros(2)
        self.t_jk_syn = np.zeros(2)
        self.t_ijpz = np.zeros(2)
        self.t_kga = np.zeros(2)
        self.t_jk = np.zeros(2)
        ...
        z_vec[:] = krylov(AZ, B.ravel(), tol=1.0e-5, max_cycle=20, x1=z_vec, verbose=5)
        if irank_shm == 0:
        ...
        az1 = None
        if irank == 0:
            #print_time(['Z vector', time_elapsed(t0)], log)
            if self.direct_int:
    ...
    if (self.wrap_test) and (self.direct_int is False):
    ...
    self.t_grad_hf = time_elapsed(t1)
    ...
    if self.solvent is not None:
        ...
        print_time(["solvation gradient", time_elapsed(t0)], log)
    ...
    for win in [win_pi, win_w, win_dm0]:
        free_win(win)
    self.t_grad_hf += time_elapsed(tt)
    return gradient
