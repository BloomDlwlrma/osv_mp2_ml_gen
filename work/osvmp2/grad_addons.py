...
from osvmp2 import hf_grad, OSVL
from osvmp2.hf_ene import scf_parallel
from osvmp2.mbe_addons import compute_energy_mbe, compute_gradient_mbe
...
class cal_para():
    ...
    def __init__(self, mol):
        self.mol_name = get_name(sys.argv[1])
        ...
        if "ml" in self.cal_mode:
            self.ml_test = True
            self.save_hf_mat = bool(int(os.environ.get("save_hf_mat", 0)))
            self.save_loc_mat = bool(int(os.environ.get("save_loc_mat", 0)))
            ...
            if os.path.isfile("ml_features.hdf5"):
                ...
                #if self.mol_name in keys:
                with h5py.File("ml_features.hdf5", "r") as f:
                    if self.mol_name in f:
                        if irank == 0:
                            print("Dataset for %s exists"%self.mol_name)
                        sys.exit()
        else:
            self.ml_mp2int = False
            self.ml_test = False
        ...
        self.wrap_test = bool(int(os.environ.get("wrap_test", 1)))
        self.use_mbe = bool(int(os.environ.get("use_mbe", 1)))
        if self.use_mbe == False:
            self.use_ga = False
        else:
            self.use_ga = bool(int(os.environ.get("use_ga", 1)))
        self.direct_int = bool(int(os.environ.get("direct_int", 0)))
        if self.direct_int is False:
            self.loc_fit = False
        else:
            self.loc_fit = bool(float(os.environ.get("loc_fit", 0)))
        self.shared_disk = bool(int(os.environ.get("shared_disk", 0))) 
        self.use_gpu = bool(int(os.environ.get("use_gpu", 0)))
        self.gpu_memory = float(os.environ.get("gpu_memory", 1100)) #MB
        self.ngpu_per_node = int(os.environ.get("ngpu_per_node", 1)) #MB
        if irank_shm < self.ngpu_per_node:
        ...
        self.idsvd_tol = float(os.environ.get("idsvd_tol", self.osv_tol))
        ...
        self.use_mbe = bool(int(os.environ.get("use_mbe", 0)))
        self.method = os.environ.get("method", "cmbeosvmp2").replace("-",'').replace("_",'').lower()
        self.basis_molpro = bool(int(os.environ.get("basis_molpro", 0)))
        if self.method == "rhf" or self.method == "hf":
            pass
        elif self.method == "cmbeosvmp2":
            self.mbe_mode = 0
        elif self.method == "mbeosvmp2":
            self.mbe_mode = 1
        elif self.method == "osvmp2":
            self.mbe_mode = 2
        else:
            raise NotImplementedError("The input method %s is not implemented"%self.method)
        #self.mbe_mode = int(os.environ.get("mbe_mode", 0))
    ...
    def get_auxbasis(self):
        atom_list = [coi[0] for coi in self.mol._atom]
        if self.basis_molpro:
            from osvmp2.get_mol_special import get_aux_molpro
        ...
        if self.use_ecp or "Be" in atom_list:
            from osvmp2.get_mol_special import get_aux
    ...
    def get_mpi_para(self):
        ...
        comm_shm.Barrier()
        self.pid_list = pid_list.tolist()
        comm_shm.Barrier()
        free_win(win_pid)

    def if_outcore(self):
        nao = self.mol.nao_nr()
        naux = max(self.naux_hf, self.naux_mp2)
        nao_pair = nao*(nao+1)//2
        #self.mol.max_eri = nao_pair*naux*8*0.9/1e6
        if self.shared_disk or self.use_ga:
            mem_total = self.mol.max_memory*self.nnode
            if (self.grad_cal):
                mem_total *= 0.3
            else:
                mem_total *= 0.4
            if nao_pair*naux*8/1e6 > mem_total:
                self.outcore = True
            else:
                self.outcore = False
        else:
            mem_per_core = self.mol.max_memory*nnode/nrank
            self.mol.max_eri = min(nao_pair*self.naux_hf*8*0.9/1e6, nao_pair*self.naux_mp2*8*0.9/1e6, mem_per_core)
            if (self.grad_cal):
                mem_com = mem_per_core
            else:
                mem_com = self.mol.max_memory/1.5 
            if nao_pair*naux*3*8/1e6 > mem_com:
                self.outcore = True
            else:
                self.outcore = False
    def extra_para(self):
...
def gradient(mol):
            ...
            if ('tmp' in f):
                try: 
                    ...
                    try: shutil.rmtree(f)
                        ...
                        pass
    def read_para(mol):
        my_para = cal_para(mol)
        my_para.extra_para()
        return my_para
    def print_info(my_para, log):
        msg = 'Basic settings:\n'
        if my_para.direct_int:
            msg += 'Integral generation: Direct\n'
        elif (my_para.outcore):
            msg += 'Integral generation: Outcore\n'
        else:
            msg += 'Integral generation: Incore\n'
        if my_para.loc_fit:
            msg += 'Local density fitting: %.1E/%.1E(block)\n'%(my_para.fit_tol, my_para.bfit_tol)
        else:
            msg += 'Local density fitting: OFF\n'
        if (my_para.shared_disk):
            msg += 'Shared disk: ON'
        else:
            msg += 'Shared disk: OFF'
        msg += "\nThe molecue to be computed: %s"%mol.name
        msg += "\nThe number of nodes: %d, cores: %d, threads/core: %s"%(nnode, nrank, os.environ.get("OMP_NUM_THREADS", 1))
        msg += "\nMaximum memory per node: %.2f MB (%.2f MB used)"%(my_para.mol.max_memory, psutil.virtual_memory()[3]*1e-6)#lib.current_memory()[0])
        msg += "\nBasis set: %s"%my_para.mol.basis
        msg += "\nAuxilary basis for HF: %s"%my_para.auxmol_hf.basis
        msg += "\nAuxilary basis for MP2: %s"%my_para.auxmol_mp2.basis
        msg += "\nThe number of AOs: %d"%my_para.mol.nao_nr()
        msg += "\nThe number of occupied MOs: %d"%(my_para.mol.nelectron//2)
        msg += "\nThe number of auxilary basis for RHF: %d"%my_para.naux_hf
        msg += "\nThe number of auxilary basis for MP2: %d\n"%my_para.naux_mp2
        if my_para.solvent is not None:
            msg += "\nCosmo solvation model with solvent "
            if type(my_para.solvent) == str:
                msg += "%s (dielectric constant = %.4f)"%(my_para.solvent, my_para.sol_eps)
            else:
                msg += "dielectric constant %.4f"%my_para.sol_eps
        if hasattr(mol, 'md_step'):
        #if my_para.cal_mode == 'md':
            if mol.md_step == 1:
    ...
    def kernel():
        ...
        clear_tmp()
        tt = time_now()
        my_para = read_para(mol)
        log = lib.logger.Logger(sys.stdout, my_para.verbose)
        if irank == 0:
            print_info(my_para, log)
        
        if my_para.qm_atoms is not None:
            win_emm, emm_node = get_shared(1)
            win_fmm, fmm_node = get_shared((mol.natm*3))
            win_charge, charge_node = get_shared(len(my_para.mm_region))
            
            if irank_shm == 0:
                GRAD_MM = qmmm.mm_gradient(my_para.coords_non_ghost, qm_region=my_para.qm_region,
                                           nonwater_region=my_para.nonwater_region, 
                                           cg_residue=my_para.cg_residue, log=log)
                #print_time(["Initialization", time_elapsed(tt)], log)
                emm_node[0], fmm, charge_node[:] = GRAD_MM.kernel()
                fmm_node.reshape(-1, 3)[my_para.index_non_ghost] = fmm.reshape(-1, 3)

            comm_shm.Barrier()
            my_para.mol_mm = pyscf.qmmm.mm_mole.create_mm_mol(my_para.mm_coords, charges=np.copy(charge_node))
        ...
        hfe = scf_parallel(hf, my_para)
        if "mp2" in my_para.method:
            mp2 = OSVL.OSVLMP2(hf, my_para)
            mp2.kernel()
            mp2e = compute_energy_mbe(mp2, log)
            e_qm = hfe + mp2e
            if my_para.grad_cal:
                g = compute_gradient_mbe(mp2, log=log).reshape(-1, 3)
                if my_para.mol_mm is not None:
                    g = get_gtot(my_para.qm_region_full, my_para.mm_region, g, g_mm, hf.mol_mm.grad)
            else:
                g = None
            t_tot = time_elapsed(tt)
            info_time(mp2, hfe, g, e_mm=e_mm, t_tot=t_tot, log=log)
        else:
            e_qm = hfe
            if my_para.grad_cal:
                hf.verbose = my_para.verbose
            ...
            #info_time(hf, hfe, g, e_mm=e_mm, g_mm=g_mm, log=log)
        e = e_qm + e_mm
        if my_para.grad_cal:
            '''if my_para.shared_disk:
                if irank == 0:
            ...
            '''if g_mm is not None:                
                my_para.mol = my_para.mol_total'''
            if irank == 0:
                if ("opt" in my_para.cal_mode):
                    ...
                    save_eg(my_para.mol_total, e, g)
                    with open("opt_traj.xyz", 'a') as f:
                        f.write(get_coords_from_mol(my_para.mol_total, info="Step %d     Energy: %.9f"%(mol.opt_cycle, e)))
                if hasattr(mol, 'md_step') and g_mm is not None:
                    ranges = []
                    for key, group in itertools.groupby(enumerate(my_para.qm_region), lambda i: i[0] - i[1]):
                        group = list(map(itemgetter(1), group))
                        ranges.append((group[0], group[-1]))
                    with open("traj_qm.xyz", 'a') as f:
                        f.write(get_coords_from_mol(my_para.mol, info="Step %d Energy: %.9f QM region: %s"%(mol.md_step, e_qm, ranges)))
        else:
            g = None
            sys.exit()
        return e, g
