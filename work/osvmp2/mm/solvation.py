...
def make_phi(pcmobj, dm, r_vdw, ui, ylm_1sph, with_nuc=True):
    ...
    win_v_phi, v_phi_node = get_shared((n_dm, natm, ngrid_1sph))
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
    ...
    win_weignts, scaled_weights = get_shared((n_dm, ngrids))
    if atom_slice is not None:
    ...
    win_den, dennode = get_shared((n_dm, ngrids))
    ...
    win_psi, psi_node = get_shared((n_dm, natm, nlm))
    if atom_slice is not None:
    ...
    win_vmat, vmat_node = get_shared((n_dm, nao, nao))
    if vmat is not None:
...
def get_veff_sol(self, dm):
    ...
    print_time(['make_phi', time_elapsed(t0)], log)
    ...
    print_time(['make_psi_vmat', time_elapsed(t0)], log)
    dielectric = self.eps
    if dielectric > 0:
    ...
    print_time(['Solvent energy and potential', time_elapsed(tt)], log)
    return epcm, vpcm
...
def make_fi(pcmobj, r_vdw):
    ...
    atom_coords = mol.atom_coords()
    ngrid_1sph = coords_1sph.shape[0]
    win_fi, fi_node = get_shared((natm,ngrid_1sph))
    job_list = []
    for ia in range(natm):
...
def make_fi1(pcmobj, r_vdw):
    ...
    atom_coords = mol.atom_coords()
    ngrid_1sph = coords_1sph.shape[0]
    win_fi1, fi1_node = get_shared((natm,3,natm,ngrid_1sph))
...
def make_L1(pcmobj, r_vdw, ylm_1sph, fi):
    ...
    win_lmat, lmat_node = get_shared((natm,3,natm,nlm,natm,nlm))
    job_list = []
    for ja in range(natm):
        for ka in atoms_with_vdw_overlap(ja, atom_coords, r_vdw):
            job_list.append([ja, ka])
...
def make_phi1(pcmobj, dm, r_vdw, ui, ylm_1sph):
    ...
    ngrid_1sph = weights_1sph.size
    win_vphi0, vphi0_node = get_shared((natm, ngrid_1sph))
...
def make_e_psi1(pcmobj, dm, r_vdw, ui, ylm_1sph, cached_pol, Xvec, L):
    ...
    win_vtmp, vtmp_node = get_shared((3, nao, nao))
    win_psi1, psi1_node = get_shared((natm,3))
    atom_slice = get_slice(range(nrank), job_size=natm)[irank]
    if atom_slice is not None:
...
def get_grad_sol(pcmobj, dm, log):
    ...
    print_time(["prepare", get_time(t0)], log)
    ...
    print_time(["L term", get_time(t0)], log)

    t0 = (logger.process_clock(), time.time())
    phi0 = make_phi(pcmobj, dm, r_vdw, ui, ylm_1sph) 
    print_time(["phi0 term", get_time(t0)], log)
    t0 = (logger.process_clock(), time.time())
    win_phi1, phi1 = make_phi1(pcmobj, dm, r_vdw, ui, ylm_1sph)
    print_time(["phi1 term", get_time(t0)], log)
    ...
    e_psi1 = make_e_psi1(pcmobj, dm, r_vdw, ui, ylm_1sph,
                         cached_pol, L0_X, L0)
    print_time(["psi term", get_time(t0)], log)
    ...
    print_time(["cal grad", get_time(t0)], log)
    comm_shm.Barrier()
